...
public class MezclaDirecta {
    ...
    public static void main(String[] args) {
        File f = new File("ArchivoOrigen.txt");
        DataOutputStream flujo = null;
        try {
            // se genera un archivo secuencialmente de claves enteras
            flujo = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(f)));
            for (int j = 1; j <= N; j++) {
            ...
            JOptionPane.showMessageDialog(null, "Error entrada/salida durante proceso" + " de ordenación ");
            e.printStackTrace();
        }
        escribir(f);
    }
    ...
    static void mezclaDirecta(File f) throws IOException {
        int longSec;
        int numReg;
        File f1 = new File("ArchivoAux1.txt");
        File f2 = new File("ArchivoAux2.txt");
        // número de registros se obtiene dividiendo el tamaño del archivo por el tamaño
        // del registro: 4.
    ...
    static void escribir(File f) {
        ...
        try {
            ...
            k = 0;
            JOptionPane.showMessageDialog(null, "ARCHIVO DE CLAVES TIPO INT");
            while (mas) {
                k++;
                JOptionPane.showMessageDialog(null, flujo.readInt() + " ");
                if (k % 11 == 0) {
                    System.out.println();
                }
            }
